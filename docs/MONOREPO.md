...
1. **Backend**: Переместить `backend/` → `apps/backend/`
2. **Frontend**: Переместить `frontend/` → `apps/mini-app/`
3. **Обновить импорты**: Использовать shared packages
4. **Обновить конфигурации**: Адаптировать под монорепозиторий
